...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Build classifier for live birth data. CANB8347 machine learning project.')
    ...
    pred = pd.read_csv(args.predict) # read in data to predict on
    pred_norm = normalize(pred, axis=1, norm='l1') # normalize each column to fractional representation between 0 and 1
    pred['dead_predict'] = rf.predict(X=pred_norm) # predict labels in prediction set and put back in pred df
